...
def load_pool(provider: str) -> CredentialPool:
    ...
    if provider.startswith(CUSTOM_POOL_PREFIX):
        ...
        # Rehydrate env-sourced entries whose access_token is empty.
        # _seed_from_env only processes vars listed in PROVIDER_REGISTRY but
        # users may add extra pool entries via `hermes auth` or auth.json with
        # source "env:SOME_VAR".  Those entries persist on disk but their
        # access_token is never populated unless we resolve it here.
        _env_file = load_env()
